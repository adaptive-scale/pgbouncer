...
char *pycall(PgSocket *client, char *username, char *query_str, int in_transaction,
	...
	slog_info(client, "Python query rewrite invoked");

	/* Load python module */
	pName = PyUnicode_FromString(py_module);
	if (pName == NULL) {
		slog_error(client, "Python module <%s> did not load from unicode", py_module);
		goto finish;
	}
	pModule = PyImport_Import(pName);
	if (pModule == NULL) {
		slog_error(client, "Python module <%s> did not load did", py_module);
		goto finish;
	}
    ...
    slog_info(client, "Python query invoked <%s> ", PyUnicode_AsUTF8String(pValue));
	...
	slog_info(client, "Python query generated <%s> ", PyUnicode_AsUTF8String(pValue));
